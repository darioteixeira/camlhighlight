...
let write ?(class_prefix = "hl_") ?(extra_classes = []) ?(dummy_lines = true) ?(linenums = false) code =
	...
	let normal_line content = XHTML.M.span ~a:[make_class ["line"]] content in
	let dummy = if dummy_lines then [XHTML.M.span ~a:[make_class ["line"; "dummy"]] []] else [] in
	...
	let convert_nums () =
		...
		in XHTML.M.p ~a:[make_class ["nums"]] (dummy @ (List.map numline_to_xhtml (List.init code_len (fun x -> x+1))) @ dummy)
	and convert_code () =
		let codeline_to_xhtml line = normal_line (List.map elem_to_xhtml line)
		in XHTML.M.p ~a:[make_class ["code"]] (dummy @ (List.map codeline_to_xhtml code) @ dummy)
